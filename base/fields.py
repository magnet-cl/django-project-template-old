...
class ReCaptchaField(forms.FileField):
    ...
    default_error_messages = {
        ...
        'incorrect-captcha-sol': (u"El texto ingresado es incorrecto, "
                                  u"por favor inténtelo de nuevo"),
        'verify-params-incorrect': (
            u"The parameters to verify were incorrect,"
            u" make sure you are passing all the required parameters."),
    ...
    def clean(self, data, initial):
        if initial is None or initial == '':
            raise Exception("ReCaptchaField requires the client's IP be set to"
                            " the initial value")
        ip = initial
